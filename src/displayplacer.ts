...
import { DisplayProfile, isDisplayProfile, Position, Profile, UserPrefs } from "./utils";
...
const buildArgsFromDisplayProfile = (display: DisplayProfile): string => {
  const collect = [
    ...
    "origin:(" + display.origin.x.toString() + "," + display.origin.y.toString() + ")",
    "degree:" + display.degree.toString(),
  ];
  return collect.join(" ");
};

export const execDisplayplacer = async (preferences: UserPrefs, args: string[]) => {
...
export const applyDisplayProfile = async (preferences: UserPrefs, profile: Profile) => {
  const args = [];
...
export const getCurrentDisplayProfile = (preferences: UserPrefs): DisplayProfile[] => {
  const executablePath = searchExecutable(preferences);
  ...
  for (const profstr of cmd.matchAll(/"(.+?)"/g)) {
    ...
    if (typeof profileObj.origin === "string") {
      const posMatch = (profileObj.origin as string).match(/^\((?<x>[0-9]+),(?<y>[0-9]+)\)$/);
