...
export const createNewConfig = async (preferences: UserPrefs) => {
  if (preferences.configfile) {
    ...
    if (configEnvs.xdgConfigHome !== '') {
      ...
      console.log(
        'Cannot create config due to missing $XDG_CONFIG_HOME and $HOME',
      )
      return ''
    }
  }
...
export const createNewConfigWithProfile = async (
...
export class DisplayplacerConfig {
  ...
  static NewConfig = (configpath?: string, profiles?: Profile[]) => {
    ...
    if (profiles)
      config.config.profiles = profiles.sort((a, b) =>
  ...
  public writeConfig = (backup: boolean = false) => {
    ...
    if (fs.existsSync(this.path)) {
      ...
      try {
        if (backup)
          fs.copyFileSync(this.path, backupPath, fs.constants.COPYFILE_EXCL)
